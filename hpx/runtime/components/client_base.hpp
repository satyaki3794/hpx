...
namespace hpx { namespace components
{
    ...
    public:
        ...
        explicit client_base(future_type const& f) HPX_NOEXCEPT
          : shared_state_(
                hpx::traits::future_access<future_type>::
                    get_shared_state(f))
        {}
        explicit client_base(future_type && f) HPX_NOEXCEPT
          : shared_state_(
                hpx::traits::future_access<future_type>::
        ...
        explicit client_base(future<id_type> && f) HPX_NOEXCEPT
          : shared_state_(hpx::traits::future_access<future_type>::
                    get_shared_state(f.share()))
        {}

        explicit client_base(client_base const& rhs) HPX_NOEXCEPT
          : shared_state_(rhs.shared_state_)
        {}
        explicit client_base(client_base && rhs) HPX_NOEXCEPT
        ...
        {
            return hpx::traits::future_access<future_type>::create(shared_state_);
        }
        ...
        ///////////////////////////////////////////////////////////////////////
    protected:
        static void register_as_helper(shared_future<id_type> && f,
    ...
    inline typename std::enable_if<
        traits::is_client<Client>::value, std::vector<Client>
    >::type
    make_client(std::vector<hpx::shared_future<hpx::id_type> > const& ids)
    {
        std::vector<Client> result;
        result.reserve(ids.size());
