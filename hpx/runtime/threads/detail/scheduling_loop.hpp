...
//         util::itt::id threadid(domain, this);
        ...
        HPX_ASSERT(scheduler.state_ != 0);

        scheduler.SchedulingPolicy::start_periodic_maintenance(*(scheduler.state_));
        ...
        while (true) {
                ...
                if (pending == state_val) {
                    ...
                    if (state_val == pending) {
                        ...
                        scheduler.SchedulingPolicy::wait_or_add_new(num_thread,
                            is_running_state(scheduler.state_->load()), idle_loop_count);
            ...
            // if nothing else has to be done either wait or terminate
                        ...
                        is_running_state(scheduler.state_->load()), idle_loop_count))
                {
                    ...
                    if (scheduler.SchedulingPolicy::cleanup_terminated(true))
                    {
                        ...
                        {
                            scheduler.state_->store(state_stopped);
                            break;
                        }
            ...
            if (*(scheduler.state_) == state_terminating)
                break;
