...
namespace hpx { namespace lcos { namespace local { namespace detail
{
    ...
    public:
        ...
        HPX_EXPORT wait(boost::unique_lock<mutex_type>& lock,
            char const* description, error_code& ec = throws);

        threads::thread_state_ex_enum
        wait(boost::unique_lock<mutex_type>& lock, error_code& ec = throws)
        {
            return wait(lock, "condition_variable::wait", ec);
        }

        threads::thread_state_ex_enum
        HPX_EXPORT wait_until(boost::unique_lock<mutex_type>& lock,
            util::steady_time_point const& abs_time,
            char const* description, error_code& ec = throws);

        threads::thread_state_ex_enum
        wait_until(boost::unique_lock<mutex_type>& lock,
            util::steady_time_point const& abs_time,
        ...
        threads::thread_state_ex_enum
        wait_for(boost::unique_lock<mutex_type>& lock,
