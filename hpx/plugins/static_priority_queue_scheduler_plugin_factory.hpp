...
namespace hpx{ namespace plugins
{
    ...
    struct static_priority_queue_scheduler_plugin_factory : public scheduler_plugin_factory_base
    {
        ...
        threads::policies::scheduler_base* create(std::size_t num_queues_=1,
            ...
            char const* description_="static_priority_queue_scheduler")
        {
            if (is_enabled_){
...
#define HPX_REGISTER_SPQ_SCHEDULER_PLUGIN_FACTORY(SchedulerPlugin, pluginname)                                            \
    HPX_REGISTER_SCHEDULER_PLUGIN_FACTORY_BASE(                                                                       \
        hpx::plugins::static_priority_queue_scheduler_plugin_factory<SchedulerPlugin>, pluginname)           \
    HPX_DEF_UNIQUE_PLUGIN_NAME(                                                                                       \
        hpx::plugins::static_priority_queue_scheduler_plugin_factory<SchedulerPlugin>, pluginname)           \
    template struct hpx::plugins::static_priority_queue_scheduler_plugin_factory<SchedulerPlugin>;           \
