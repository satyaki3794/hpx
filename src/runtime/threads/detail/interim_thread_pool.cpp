...
{
    ///////////////////////////////////////////////////////////////////////////
    void interim_thread_pool::init_tss(std::size_t num)
    {
        thread_num_tss_.init_tss(num);
    }

    void interim_thread_pool::deinit_tss()
    {
        thread_num_tss_.deinit_tss();
    }

    ///////////////////////////////////////////////////////////////////////////
    interim_thread_pool::interim_thread_pool(std::shared_ptr<threads::policies::scheduler_base> sched,
    ...
    interim_thread_pool::~interim_thread_pool()
    {
        if (!threads_.empty()) {
    ...
    ///////////////////////////////////////////////////////////////////////////
    hpx::state interim_thread_pool::get_state() const
    {
    ...
    hpx::state interim_thread_pool::get_state(std::size_t num_thread) const
    {
        HPX_ASSERT(num_thread != std::size_t(-1));
        return (*sched_).get_state(num_thread).load();
    }

    bool interim_thread_pool::has_reached_state(hpx::state s) const
    {
        return (*sched_).has_reached_state(s);
    }

    ///////////////////////////////////////////////////////////////////////////
    std::size_t interim_thread_pool::init(std::size_t num_threads,
        policies::init_affinity_data const& data)
    {
        topology const& topology_ = get_topology();
        std::size_t cores_used = (*sched_).init(data, topology_);

        resize(used_processing_units_, threads::hardware_concurrency());
        for (std::size_t i = 0; i != num_threads; ++i)
            used_processing_units_ |= (*sched_).get_pu_mask(topology_, i);

        return cores_used;
    }

    ///////////////////////////////////////////////////////////////////////////
    std::size_t interim_thread_pool::get_pu_num(std::size_t num_thread) const
    {
        return (*sched_).get_pu_num(num_thread);
    }

    mask_cref_type interim_thread_pool::get_pu_mask(
        topology const& topology, std::size_t num_thread) const
    {
        return (*sched_).get_pu_mask(topology, num_thread);
    }

    mask_cref_type interim_thread_pool::get_used_processing_units() const
    {
        return used_processing_units_;
    }

    void interim_thread_pool::do_some_work(std::size_t num_thread)
    {
        (*sched_).do_some_work(num_thread);
    }

    void interim_thread_pool::report_error(std::size_t num,
        boost::exception_ptr const& e)
    {
        (*sched_).set_all_states(state_terminating);
        notifier_.on_error(num, e);
        (*sched_).on_error(num, e);
    }

    ///////////////////////////////////////////////////////////////////////////
    void interim_thread_pool::create_thread(thread_init_data& data,
        ...
        error_code& ec)
    {
        ...
        if (thread_count_ == 0 && !(*sched_).is_state(state_running))
        {
            ...
            HPX_THROWS_IF(ec, invalid_status,
                "interim_thread_pool::create_thread",
    ...
    void interim_thread_pool::create_work(thread_init_data& data,
        thread_state_enum initial_state, error_code& ec)
    {
        ...
        if (thread_count_ == 0 && !(*sched_).is_state(state_running))
        {
            // thread-manager is not currently running
            HPX_THROWS_IF(ec, invalid_status,
                "interim_thread_pool::create_work",
                "invalid state: thread pool is not running");
            return;
        }
    ...
    thread_state interim_thread_pool::set_state(
    ...
    thread_id_type interim_thread_pool::set_state(
    ...
    void interim_thread_pool::abort_all_suspended_threads()
    {
        (*sched_).abort_all_suspended_threads();
    }

    bool interim_thread_pool::cleanup_terminated(bool delete_all)
    {
        return (*sched_).cleanup_terminated(delete_all);
    }

    ///////////////////////////////////////////////////////////////////////////
    std::size_t interim_thread_pool::get_worker_thread_num() const
    {
        return thread_num_tss_.get_worker_thread_num();
    }

    boost::thread& interim_thread_pool::get_os_thread_handle(
    ...
    std::int64_t interim_thread_pool::get_thread_count(
        thread_state_enum state, thread_priority priority,
        std::size_t num, bool reset) const
    {
        return (*sched_).get_thread_count(state, priority, num, reset);
    }

    void interim_thread_pool::reset_thread_distribution()
    {
        return (*sched_).reset_thread_distribution();
    }

    void interim_thread_pool::set_scheduler_mode(
    ...
    ///////////////////////////////////////////////////////////////////////////
    bool interim_thread_pool::run(std::unique_lock<boost::mutex>& l,
        std::size_t num_threads)
    {
        HPX_ASSERT(l.owns_lock());

        LTM_(info) //-V128
            << "interim_thread_pool::run: " << pool_name_
        ...
        if (0 == num_threads) {
            HPX_THROW_EXCEPTION(bad_parameter,
                "interim_thread_pool::run", "number of threads is zero");
        }
        ...
        try {
            ...
            while (thread_num-- != 0) {
                threads::mask_cref_type mask =
                    (*sched_).get_pu_mask(topology_, thread_num);
    ...
    (defined(HPX_HAVE_MAX_CPU_COUNT) && HPX_HAVE_MAX_CPU_COUNT <= 64)
                ...
                threads_.push_back(boost::thread(
                        &interim_thread_pool::thread_func, this, thread_num,
        ...
        LTM_(info) << "interim_thread_pool::run: " << pool_name_ << " running";
        return true;
    }

    ///////////////////////////////////////////////////////////////////////////
    void interim_thread_pool::stop (
    ...
    void interim_thread_pool::stop_locked(Lock& l, bool blocking)
    {
        LTM_(info)
            << "interim_thread_pool::stop: " << pool_name_
        ...
        if (!threads_.empty()) {
            ...
            (*sched_).do_some_work(std::size_t(-1));
    ...
    {
        init_tss_helper(interim_thread_pool& pool, std::size_t thread_num)
          : pool_(pool), thread_num_(thread_num)
        {
            pool.notifier_.on_start_thread(thread_num);
            pool.init_tss(thread_num);
            (*pool.sched_).on_start_thread(thread_num);
        }
        ~init_tss_helper()
        {
            (*pool_.sched_).on_stop_thread(thread_num_);
            pool_.deinit_tss();
            pool_.notifier_.on_stop_thread(thread_num_);
        }

        interim_thread_pool& pool_;
        std::size_t thread_num_;
    };

    void interim_thread_pool::thread_func(std::size_t num_thread,
        topology const& topology, boost::barrier& startup)
    {
        // Set the affinity for the current thread.
        threads::mask_cref_type mask =
            (*sched_).get_pu_mask(topology, num_thread);
        ...
        if (any(mask))
        {
            ...
            {
                LTM_(warning) //-V128
                    << "interim_thread_pool::thread_func: " << pool_name_
                    << " setting thread affinity on OS thread " //-V128
        ...
        init_tss_helper tss_helper(*this, num_thread);
...

        {
            ...
            try {
                try {
                    ...
                    HPX_ASSERT(!(*sched_).get_thread_count(
...
#if defined(HPX_HAVE_THREAD_CUMULATIVE_COUNTS)
    std::int64_t interim_thread_pool::
...
#if defined(HPX_HAVE_THREAD_IDLE_RATES)
    ///////////////////////////////////////////////////////////////////////////
    std::int64_t interim_thread_pool::avg_idle_rate(bool reset)
    {
    ...
    std::int64_t interim_thread_pool::avg_idle_rate(
...
#if defined(HPX_HAVE_THREAD_CREATION_AND_CLEANUP_RATES)
    std::int64_t interim_thread_pool::avg_creation_idle_rate(bool reset)
    {
    ...
    std::int64_t interim_thread_pool::avg_cleanup_idle_rate(bool reset)
    {
    ...
    {
        return (*sched_).get_queue_length(num_thread);
    }
    ...
    {
        return (*sched_).get_average_thread_wait_time(num_thread);
    }

    std::int64_t interim_thread_pool::
        get_average_task_wait_time(std::size_t num_thread) const
    {
        return (*sched_).get_average_task_wait_time(num_thread);
    }
#endif
    ...
    {
        return (*sched_).get_num_pending_misses(num, reset);
    }

    std::int64_t interim_thread_pool::
        get_num_pending_accesses(std::size_t num, bool reset)
    {
        return (*sched_).get_num_pending_accesses(num, reset);
    }

    std::int64_t interim_thread_pool::
        get_num_stolen_from_pending(std::size_t num, bool reset)
    {
        return (*sched_).get_num_stolen_from_pending(num, reset);
    }

    std::int64_t interim_thread_pool::
        get_num_stolen_to_pending(std::size_t num, bool reset)
    {
        return (*sched_).get_num_stolen_to_pending(num, reset);
    }

    std::int64_t interim_thread_pool::
        get_num_stolen_from_staged(std::size_t num, bool reset)
    {
        return (*sched_).get_num_stolen_from_staged(num, reset);
    }

    std::int64_t interim_thread_pool::
        get_num_stolen_to_staged(std::size_t num, bool reset)
    {
        return (*sched_).get_num_stolen_to_staged(num, reset);
    }
#endif
...
///////////////////////////////////////////////////////////////////////////////
/// explicit instantiation for the thread manager of our choice
// #include <hpx/runtime/threads/policies/scheduler_base.hpp>
// class HPX_EXPORT hpx::threads::detail::interim_thread_pool;
